/**
 * @preserve jQuery PeriodPicker plugin v3.9.9
...
(function ($, window, document) {
    ...
    function PeriodPicker(startinput, options) {
        ...
        that.picker = $('<div unselectable="on" class="period_picker_box xdsoft_noselect ' + (options.fullsize ? 'period_picker_maximize' : '') + '" style="">' +
            ...
            '<div class="period_picker_head">' +
                ...
                ((options.closeButton && !options.inline) ? '<span class="period_picker_close" title="' + this.i18n('Close') + '"></span>' : '') +
            '</div>' +
            ...
            '<div class="period_picker_submit_dates">' +
                ((that.options.timepicker && $.fn.TimePicker !== undefined) ? '<span style="' + (that.options.showTimepickerInputs ? '' : 'visibility:hidden') + '" class="period_picker_from_time_block period_picker_time">' +
                    '<span class="input_box"><input data-index="0"  class="input_control period_picker_from_time"></span>' +
                '</span>' : '') +
                '<span style="' + (that.options.showDatepickerInputs ? '' : 'visibility:hidden') + '" class="period_picker_from_block period_picker_date">' +
                    '<span class="input_box"><input class="input_control period_picker_from" maxlength="10"></span>' +
                '</span>' +
                '<span style="' + (that.options.showDatepickerInputs ? '' : 'visibility:hidden') + '">&#8212;</span>' +
                '<span style="' + (that.options.showDatepickerInputs ? '' : 'visibility:hidden') + '" class="period_picker_to_block period_picker_date">' +
                    '<span class="input_box"><input class="input_control period_picker_to" maxlength="10"></span>' +
                '</span>' +
                ((that.options.timepicker && $.fn.TimePicker !== undefined && !that.options.norange) ? '<span style="' + (that.options.showTimepickerInputs ? '' : 'visibility:hidden') + '" class="period_picker_to_time_block period_picker_time">' +
                    '<span class="input_box"><input data-index="1" class="input_control period_picker_to_time"></span>' +
                '</span>' : '') +
                ((options.okButton && !options.inline) ? '<button class="period_picker_show period_picker_ok" role="button" type="button">' +
                    '<span class="button_text">' + this.i18n('OK') + '</span>' +
                '</button>' : '') +
                (options.clearButton ? '<button class="period_picker_show period_picker_clear" role="button" type="button">' +
                    '<span class="button_text">' + this.i18n('Clear') + '</span>' +
                '</button>' : '') +
            '</div>' +
            '</div>');

        if (that.options.withoutBottomPanel || (!that.options.clearButton && (!that.options.okButton || that.options.inline) && !that.options.showDatepickerInputs && (!that.options.showTimepickerInputs || !that.options.timepicker || $.fn.TimePicker === undefined))) {
            that.picker.addClass('without_bottom_panel');
            that.options.withoutBottomPanel = true;
            that.options.someYOffset = 0;
        }

        if (!that.options.yearsLine) {
            that.picker.addClass('without_yearsline');
        }

        if (!that.options.title && !that.options.fullsizeButton && !(options.closeButton && !options.inline)) {
            ...
            '<span class="period_button_text">' +
                '<span class="period_button_content_wrapper">' +
                    '<span class="period_button_content">' +
                        ...
                        (that.options.clearButtonInButton ? '<span class="icon_clear"></span>' : '') +
                    '</span>' +
                '</span>' +
            '</span>' +
            '</div>');
        ...
        that.addRange([date.parseStr(that.startinput.val(), options.formatDateTime) || date.parseStr(that.startinput.val(), options.formatDate), date.parseStr(that.endinput.val(), options.formatDateTime) || date.parseStr(that.endinput.val(), options.formatDate)]);
    ...
    PeriodPicker.prototype.setInputsValue = function () {
        ...
        if (result.length) {
            ...
            if (result[1] && this.endinput.length && this.endinput.val() !== result[1]) {
                this.endinput.val(result[1]).trigger('change');
            }
        } else {
            this.startinput.val('').trigger('change');
            this.endinput.val('').trigger('change');
        }
    };
    ...
    PeriodPicker.prototype.init = function () {
        ...
        that.picker.on('click', '.period_picker_show.period_picker_ok', function () {
            ...
            if (that.options.onOkButtonClick && $.isFunction(that.options.onOkButtonClick)) {
                that.options.onOkButtonClick.call(that);
            }
        });
    ...
    PeriodPicker.prototype.generateCalendarLine = function (month, year, count) {
        ...
        this.calcDate(date, year, month - 1, 1);
    ...
    PeriodPicker.prototype.show = function (target) {
        ...
        if (!this.options.inline) {
            ...
            } else if (!this.picker.hasClass('xdsoft_i_moved')) {
                offset = !this.options.likeXDSoftDateTimePicker ? this.button.offset() : $(target).offset();
    ...
    PeriodPicker.prototype.destroy = function () {
        this.picker.remove();
        this.button.remove();
        this.startinput.off('.xdsoftpp').show();
